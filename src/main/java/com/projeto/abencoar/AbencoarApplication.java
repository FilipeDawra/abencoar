...
public class AbencoarApplication {
	...
	public CommandLineRunner carregarDadosIniciais(
			...
			PlatformTransactionManager transactionManager) { // 🚀 Gerenciador injetado com sucesso

		return args -> {
			// Criando o executor programático de transações blindadas
			...
			transactionTemplate.execute(status -> {
				...
				if (beneficiarioRepository.count() == 0) {
					...
					// 🚀 Corrigido: Trocado saveAndFlush por save comum
				...
				if (gradeRepository.count() <= 1) {
					...
					// 🚀 Corrigido: Removido o .flush() desnecessário daqui
				...
				// 🚀 Corrigido: Removidos entityManager.flush() e clear() para não quebrar o ciclo transacional das lambdas
				...
				if (masso != null) {
					...
					if (!horariosLivres.isEmpty()) {
						...
						if (filipe != null) {
							...
							// 🚀 Corrigido: Mantido o .save() limpo e comum
				...
				System.out.println("====== [DEBUG] FIM DA CARGA DE DADOS EM TRANSAÇÃO SEGURA ======");
				return null; // Retorno padrão exigido pelo TransactionTemplate
			});
		};
	}
}